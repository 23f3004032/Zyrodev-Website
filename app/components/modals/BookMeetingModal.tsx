...
export default function BookMeetingModal({ isOpen, onClose }: BookMeetingModalProps) {
  ...
  // Custom Calendar (Preferred Date) state variables
  ...
  // Effect to manage the modal's dedicated Lenis instance for scrolling on desktop
  useEffect(() => {
  ...
  const handleDateSelect = (day: number) => {
    ...
    setSelectedDate(d);
    setCalendarOpen(false); // Close calendar overlay instantly on date select
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Combine date and time to construct preferredDateTime parameter
    ...
    if (selectedDate) {
      ...
      const dateString = `${selectedDate.getFullYear()}-${monthStr}-${dayStr}`;
      preferredDateTime = `${dateString} ${selectedHour}:${selectedMinute} ${selectedPeriod}`;
    }
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
              ...
              {/* Row 2: Custom Preferred Date Selector Trigger */}
              <div className={`text-left relative transition-all duration-300 ${calendarOpen ? 'pb-[200px] md:pb-[160px]' : ''}`}>
                <label className="block text-slate-400 font-mono text-xs uppercase tracking-wider mb-2 select-none">
                  Preferred Date *
                </label>
                ...
                >
                  <span className={selectedDate ? 'text-slate-300 font-bold' : 'text-slate-500'}>
                    {selectedDate 
                ...
                {/* Custom Datepicker Popover Card */}
                {calendarOpen && (
                  <>
                    {/* Backdrop close capture */}
              ...
              {/* Row 3: Dedicated Time Select Inputs */}
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <select 
                      ...
                      className="w-full bg-black/50 border border-slate-800 rounded px-2.5 py-2.5 font-mono text-sm text-slate-300 focus:outline-none focus:border-cyan-500 cursor-pointer"
                    >
